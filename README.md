...
2. Unzip it and open `install_EcoAssist.command`. This command will probably not open when double-clicked because your computer won't trust it. Right-click > Open > Open will circumvent that.
...
- Please make sure that the old EcoAssist files are deleted before installing version 2. So go ahead and delete the old `EcoAssist_files` directory and the `open_EcoAssist.command` wherever you placed it. Don't worry if you forgot this prior to installing v2, it just means that you have two versions of EcoAssist installed. You can also delete it afterwards.

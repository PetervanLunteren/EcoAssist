...
def separate_images(path_to_image_folder):
    ...
    for image in data['images']:  # mkdir for all dirs containing images
        ...
        file_name = os.path.splitext(os.path.basename(os.path.normpath(file[0]+file[1])))  # list: ext in [1] and the rest in [0]
        detections_list = image['detections']
...
def separate_videos(path_to_video_folder):
    ...
    detections_dict = {}
    for image in data['images']:
        video, frame = os.path.normpath(image['file']).split(os.sep)
        detections_list = image['detections']
        n_detections = len(detections_list)
    ...
    for video in detections_dict:
        ...
        if detections_dict[video][0] != 0 and detections_dict[video][1] == 0 and detections_dict[video][2] == 0:
            ...
            Path(os.path.join(path_to_video_folder, "videos", "multiple_categories")).mkdir(parents=True, exist_ok=True)
            os.replace(os.path.join(path_to_video_folder, video),
...
def openProgressWindow():
    ...
    if data_type.get() == "Images":
        ...
        if check_cont_from_checkpoint.get() and check_if_checkpointfile_is_present(
            ...
            # Megadetector status
            if check_prod_JSON:
                mega_frame = LabelFrame(newWindow, text="Algorithm", pady=2, padx=5, relief='solid', highlightthickness=5,
            ...
            # crop status
            if check_crop.get():
                crop_frame = LabelFrame(newWindow, text="Crop images", pady=2, padx=5, relief='solid', highlightthickness=5,
        ...
        try:
            ...
            if v_check_dont_process_every_frame:
...
t.insert(END, "Here you can browse for a folder which contains camera trap images or video\'s. All further specified settings will be performed on this directory.\n\n")
...
t.insert(END, "This is a functionality to save results to checkpoints intermittently, in case technical hiccup arises. That way you won't have to restart the entire process again when the process is interrupted.\n\n")
t.tag_add('title', '24.0', '24.end')
t.tag_add('info', '25.0', '25.end')

t.insert(END, "Checkpoint frequency\n")
t.insert(END, "Fill in how often you want to save the results to checkpoints. The number indicates the number of images after which checkpoints will be saved. The entry must contain only numeric characters.\n\n")
t.tag_add('title', '27.0', '27.end')
t.tag_add('info', '28.0', '28.end')

t.insert(END, "Continue from last checkpoint file onwards\n")
t.insert(END, "Here you can choose to continue from the last saved checkpoint onwards so that the algorithm can continue where it left off. Checkpoints are saved into the 'json' subdirectory within the main folder.\n\n")
t.tag_add('title', '30.0', '30.end')
t.tag_add('info', '31.0', '31.end')

t.insert(END, "Draw boxes around detections\n")
t.insert(END, "This functionality draws boxes around the detections and saves them in the subdirectory '_visualised_images'. Animals, persons and vehicles are visualised using different colours.\n\n")
...
t.insert(END, "The crop and draw bounding box functions alter the images. Specify if you want to delete the unaltered orignal images.\n\n")
...
t.insert(END, "This function divides the images in the subdirectories 'empties', 'animals', 'persons', 'vehicles', and 'multiple_categories'.\n\n")
...
t.insert(END, "When processing every frame of a video, it can take a long, long time to finish. Here you can specify whether you want to analyse only a selection of frames. At 'analyse every Nth frame' you can specify how many frames you want to be analysed.\n\n")
t.tag_add('title', '63.0', '63.end')
t.tag_add('info', '64.0', '64.end')

t.insert(END, "Analyse every Nth frame\n")
t.insert(END, "Specify how many frames you want to process. By entering 2, you will process every 2nd frame and thus cut process time by half. By entering 50, you will shorten process time to 1/50, et cetera.\n\n")
...
t.insert(END, "This function divides the videos in the subdirectories 'empties', 'animals', 'persons', 'vehicles', and 'multiple_categories'.")
t.tag_add('title', '69.0', '69.end')
t.tag_add('info', '70.0', '70.end')
...
text.insert(END, "EcoAssist is a freely available and open-source application with the aim of helping ecologists all over the world with their camera trap imagery. It uses a deep learning algorithm trained to detect the presence of animals, people and vehicles in camera trap data. You can let the algorithm process images or videos and filter out the empties, annotate for further processing (such as training your own algorithm) and visualise or crop the detections.\n\n")
